...
def load_traindata(processing, classes, partial_data, naVal, fillna=True, numdata=1000, resizex=320, resizey=320):
    ...
    classesdf = traindf[classes].fillna(naVal).iloc[:100000] # ADJUSTED
    # paths = traindf["Path"].tolist()[:-1]    
    paths = traindf["Path"].iloc[:100000].tolist()
    if partial_data:
...
if __name__ == "__main__":
    ...
    filename = "/home/bjuarez/CS156b/predictions/inception_100000_" + processing + "_" + sys.argv[2] + "_" + sys.argv[3] + "_" + sys.argv[4] + "_" + sys.argv[5] + ".csv"
    batch_size = 64
